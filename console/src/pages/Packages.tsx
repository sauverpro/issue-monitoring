    ...
    <div>
      ...
      <div className="mb-6 space-y-2 rounded-2xl border border-zinc-200 bg-white p-5 text-sm leading-relaxed text-zinc-600 dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-400">
        ...
        <p>
          ...
          (npm 12+ blocks tarball URLs otherwise — that is the{" "}
          <code>EALLOWREMOTE</code> error). Install <em>core together with</em> web or React Native
          so npm does not look up <code>@koralink/monitor-core</code> on npmjs. Same-machine
          alternative: download the <code>.tgz</code> and{" "}
          <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-800">npm install ./file.tgz</code>{" "}
          (no flag). GitHub Packages: <code>docs/monitor/PACKAGES.md</code>.
        </p>
        <p>
          <strong className="text-zinc-900 dark:text-zinc-100">3. Prefer HTTP ingest</strong> on the
          project Integration page (curl / fetch / axios). Use these tarballs only if you want
          automatic hooks.
        </p>
      </div>
      {error && (
      ...
      <Panel>
        {pkgs.length === 0 && !error ? (
          <p className="px-5 py-8 text-center text-sm text-zinc-500">No tarballs in packages/releases yet.</p>
        ) : (
          <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {pkgs.map((p) => {
              const cmd = installCmd(p);
              return (
                <li key={p.file} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center">
