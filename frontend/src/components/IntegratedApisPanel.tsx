...
export function IntegratedApisPanel({
  ...
  if (loading && rows.length === 0) {
    return (
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-6">
        ...
        <div className="mt-4 h-32 animate-pulse rounded-lg bg-zinc-900/60" />
      </section>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <section className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900/20 px-4 py-6">
        <h2 className="text-sm font-medium text-zinc-400">Integrated APIs</h2>
        <p className="mt-2 text-sm text-zinc-500">
          No upstream keys registered yet. Send <code className="text-zinc-400">upstream_key</code>{" "}
          on each <code className="text-zinc-400">POST /events</code> payload (one stable id per
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">
          ...
          <span className="text-zinc-500">
            ...
            <code className="text-zinc-400">status_code: 0</code> or{" "}
            <code className="text-zinc-400">outcome: &quot;OTHER&quot;</code> when the server never
            responded.
          </span>
        </p>
      </div>
        ...
        <table className="min-w-full text-left text-xs sm:text-sm">
          ...
          <tbody className="divide-y divide-zinc-800">
            {sorted.map((r) => (
              ...
              >
                <td className="whitespace-nowrap px-3 py-2.5 font-medium text-zinc-300">
                  {r.service}
                </td>
                <td className="max-w-[220px] px-3 py-2.5 font-mono text-emerald-400/90">
