  ...
  if (loading && rows.length === 0) {
    return (
      <section className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/30 p-6">
        ...
        <div className="mt-4 h-40 animate-pulse rounded-lg bg-zinc-50/60 dark:bg-zinc-900/60" />
      </section>
    );
  }
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-500">
          ...
          <code className="text-zinc-700 dark:text-zinc-400">TRACKED_*_URL</code> in <code className="text-zinc-700 dark:text-zinc-400">.env</code>.
        </p>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/30">
        <table className="min-w-full text-left text-xs sm:text-sm">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {rows.map((r) => (
              <tr key={r.id} className="hover:bg-zinc-50/40 dark:hover:bg-zinc-900/40">
                <td className="px-3 py-2.5">
                  <div className="font-medium text-zinc-700 dark:text-zinc-200">{r.label}</div>
                  <div className="mt-0.5 font-mono text-[11px] text-zinc-600 dark:text-zinc-500">{r.id}</div>
                </td>
                <td className="max-w-[280px] px-3 py-2.5">
                  <a
                    ...
                    className="inline-flex items-center gap-1 break-all text-cyan-600/90 dark:text-cyan-400/90 hover:text-cyan-700 dark:hover:text-cyan-300"
                  >
                    {r.base_url}
