    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <input
          ...
          className="rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-800 dark:text-zinc-100"
        />
        <input
          value={email}
          onChange={(e) => {
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {loading ? (
              ...
              items.map((s) => (
                <tr key={s.sessionId} className="hover:bg-zinc-50/40 dark:hover:bg-zinc-900/40">
                  <td className="max-w-[160px] truncate px-3 py-2 font-mono text-xs text-zinc-600 dark:text-zinc-300">
                    {s.sessionId}
                  </td>
                  <td className="px-3 py-2 text-zinc-600 dark:text-zinc-300">{s.userEmail ?? "—"}</td>
                  <td className="px-3 py-2 text-zinc-600 dark:text-zinc-500">{s.role ?? "—"}</td>
                  <td className="px-3 py-2 text-zinc-600 dark:text-zinc-500">{s.accountType ?? "—"}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{s.totalActions}</td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${
                      s.failures > 0 ? "text-red-700 dark:text-red-300" : "text-zinc-600 dark:text-zinc-500"
                    }`}
                  >
                    {s.failures}
                  </td>
