...
export function SessionDetail() {
  ...
  if (loading) {
    return <p className="text-zinc-500">Loading session…</p>;
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-4 text-sm text-zinc-400">
          ...
          {session.role && (
            <span>
              Role: <span className="text-zinc-200">{session.role}</span>
            </span>
          )}
          {session.account_type && (
            <span>
              Account:{" "}
              <span className="text-zinc-200">{session.account_type}</span>
            </span>
          )}
          <span>
            ...
            <span
              className={
                session.failure_events > 0 ? "text-red-300" : "text-zinc-300"
              }
            >
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-800">
            {timeline.length === 0 ? (
              ...
              timeline.map((row) => (
                <tr key={row.id} className="align-top font-mono text-xs hover:bg-zinc-900/40">
                  ...
                  <td
                    className="max-w-[320px] truncate px-3 py-2 text-emerald-400/90"
                    title={row.request_url ?? row.endpoint}
                  >
                    {row.request_url ?? row.endpoint}
                  </td>
                  <td className="max-w-[200px] truncate px-3 py-2 text-red-300/80">
                    {row.failure_reason ?? "—"}
                  </td>
