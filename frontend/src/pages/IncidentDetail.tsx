  ...
  if (err && !data) {
    return (
      <p className="text-red-400">
        {err}{" "}
        <Link to="/incidents" className="text-emerald-400 underline">
          Back
        </Link>
      </p>
    );
      ...
      <header>
        ...
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <div>
            ...
            <dd className="font-medium text-zinc-200">{incident.service}</dd>
          </div>
          <div>
            <dt className="text-zinc-500">Severity</dt>
            <dd className="font-medium text-zinc-200">{incident.severity}</dd>
          </div>
          <div>
            <dt className="text-zinc-500">Status</dt>
            <dd className="font-medium text-zinc-200">{incident.status}</dd>
          </div>
          <div>
          ...
          {incident.trigger_error_rate && (
            <div>
              <dt className="text-zinc-500">Trigger error rate</dt>
            ...
            <div>
              ...
              <dd className="text-zinc-300">
                ...
                {incident.auto_resolved && (
                  <span className="ml-2 text-xs text-zinc-500">(auto)</span>
                )}
              </dd>
            </div>
          )}
      ...
      <section>
        ...
        <form onSubmit={addNote} className="mt-4 space-y-2">
          <textarea
            ...
            className="w-full rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-white outline-none ring-emerald-500/30 focus:ring-2"
          />
          <button
            type="submit"
