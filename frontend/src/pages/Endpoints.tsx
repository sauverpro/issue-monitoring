  ...
  return (
    <div className="space-y-6">
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-800">
            {loading ? (
              ...
              endpoints.map((ep) => (
                <tr key={ep.endpoint_key} className="hover:bg-zinc-900/40">
                  <td
                    className="max-w-[360px] truncate px-3 py-2 font-mono text-xs text-emerald-400/90"
      ...
      <p className="text-xs text-zinc-500">
        ...
        <Link to="/events?outcome=FAILURE" className="text-emerald-400 hover:text-emerald-300">
          event log
        </Link>{" "}
        or open a session from the{" "}
        <Link to="/sessions?has_failures=true" className="inline-flex items-center gap-0.5 text-emerald-400 hover:text-emerald-300">
          sessions list
